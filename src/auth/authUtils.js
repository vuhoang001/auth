...
const HEADER = {
    API_KEY: "x-api-key", CLIENT_ID: "x-client-id", AUTHORIZATION: "authorization", REFRESHTOKEN: "x-rtoken-id",
};
...
const createTokensPair = async (payload) => {
    try {
        ...
        const rtokenTime = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET);
        const atokenTime = jwt.verify(accessToken, process.env.ACCESS_TOKEN_SECRET);
        return {
            accessToken, refreshToken, rtokenExp: rtokenTime.exp, atokenExp: atokenTime.exp,
        };
    } catch (error) {
        return error;
    }
...
const authentication = AsyncHandle(async (req, res, next) => {
    ...
    if (!refreshToken && !accessToken) throw new AuthFailureError("Invalid token");
    if (refreshToken) {
...
const checkPermission = (roles) => AsyncHandle(async (req, res, next) => {
    const user = req.user;
...
module.exports = {
    createTokensPair, authentication, checkStatusProject, checkPermission,
};
